...
import type { SystemSettings, SysInfo, Project, ObsidianVault, ObsidianDiscoveredVault } from '../lib/api'
...
function ObsidianTab() {
  ...
  useEffect(() => {
    Promise.all([api.admin.getSettings(), api.obsidian.listVaults()])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3">
        ...
        <p className="text-xs text-slate-500">The folder that contains all your Obsidian vaults, e.g. /Users/jon/vaults</p>
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-4 py-2 text-sm font-medium rounded-lg bg-violet-600 hover:bg-violet-500 disabled:opacity-50 text-white transition-colors"
          >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-slate-300">Auto-write after task completion</p>
            <p className="text-xs text-slate-500 mt-0.5">
              When enabled, Phoenix generates and saves an Obsidian note automatically after every completed task.
            </p>
          </div>
          <button
            onClick={() => saveSettings({ obsidian_auto_write: !settings.obsidian_auto_write })}
            disabled={saving}
            className={`relative w-11 h-6 rounded-full transition-colors ${
              settings.obsidian_auto_write ? 'bg-violet-600' : 'bg-slate-700'
            }`}
          >
            <span className={`absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white shadow transition-transform ${
              settings.obsidian_auto_write ? 'translate-x-5' : 'translate-x-0'
            }`} />
          </button>
        </div>
      </div>
      ...
      {discovered.filter(d => !d.configured).length > 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-3">
          <h3 className="text-sm font-semibold text-slate-300">Discovered Vaults</h3>
          <div className="space-y-2">
            {discovered.filter(d => !d.configured).map(d => (
              <div key={d.path} className="flex items-center justify-between py-2 px-3 bg-slate-800/60 rounded-lg">
                <div>
                  <p className="text-sm text-white font-medium">{d.name}</p>
                  <p className="text-xs text-slate-500 font-mono">{d.path}</p>
                </div>
                <button
                  onClick={() => addVault(d)}
                  disabled={addingVaultId === d.path}
                  className="px-3 py-1.5 text-xs font-medium rounded-lg bg-violet-600/20 text-violet-400 border border-violet-600/30 hover:bg-violet-600/30 disabled:opacity-50 transition-colors"
                >
      ...
      {vaults.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-semibold text-slate-300">Configured Vaults</h3>
          {vaults.map(vault => (
            <VaultCard
              key={vault.id}
              vault={vault}
      ...
      {vaults.length === 0 && discovered.length === 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center">
          <div className="text-3xl mb-2">🗂</div>
          <p className="text-slate-400 text-sm">No vaults configured yet.</p>
...
function VaultCard({ vault, generatingContext, onContextChange, onGenerateContext, onToggle, onDelete }: VaultCardProps) {
  ...
  return (
    <div className={`bg-slate-900 border rounded-xl p-4 space-y-3 transition-opacity ${vault.enabled ? 'border-slate-800' : 'border-slate-800/50 opacity-60'}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-white">{vault.name}</p>
          <p className="text-xs text-slate-500 font-mono truncate">{vault.path}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={onDelete} className="text-xs text-slate-600 hover:text-red-400 transition-colors px-1">✕</button>
        </div>
      </div>

      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <label className="text-xs font-medium text-slate-400">Context description</label>
          <button
        ...
        <textarea
          ...
          className="w-full bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white placeholder-slate-500 resize-none focus:outline-none focus:ring-2 focus:ring-violet-500"
        />
      </div>
    </div>
  )
