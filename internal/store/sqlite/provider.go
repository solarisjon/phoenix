...
func (r *ProviderRepo) List(ctx context.Context, userID string) ([]*model.Provider, error) {
	...
	if userID == "" {
		rows, err = r.db.QueryContext(ctx,
			`SELECT id, name, type, config, allowed_models, created_by, created_at,
...
func (r *ProviderRepo) Create(ctx context.Context, p *model.Provider) error {
	modelsJSON, err := marshalModelEntries(p.AllowedModels)
	if err != nil {
		return fmt.Errorf("marshal allowed_models: %w", err)
	}
	_, err = r.db.ExecContext(ctx,
		`INSERT INTO providers (id, name, type, config, allowed_models, created_by) VALUES (?, ?, ?, ?, ?, ?)`,
		p.ID, p.Name, string(p.Type), p.Config, modelsJSON, p.CreatedBy)
	if err != nil {
...
func (r *ProviderRepo) Update(ctx context.Context, p *model.Provider) error {
	...
	_, err = r.db.ExecContext(ctx,
		`UPDATE providers SET name = ?, type = ?, config = ?, allowed_models = ? WHERE id = ?`,
		p.Name, string(p.Type), p.Config, modelsJSON, p.ID)
	if err != nil {
...
func (r *ProviderRepo) UpdateAllowedModels(ctx context.Context, id string, models []model.ModelEntry) error {
	modelsJSON, err := marshalModelEntries(models)
	if err != nil {
		return fmt.Errorf("marshal allowed_models: %w", err)
	}
	_, err = r.db.ExecContext(ctx, `UPDATE providers SET allowed_models = ? WHERE id = ?`, modelsJSON, id)
	if err != nil {
...
func scanProvider(row *sql.Row) (*model.Provider, error) {
	...
	var typ, allowedModelsJSON string
	var latencyMs sql.NullInt64
	var checkedAt sql.NullTime
	err := row.Scan(&p.ID, &p.Name, &typ, &p.Config, &allowedModelsJSON, &p.CreatedBy, &p.CreatedAt,
	...
	p.AllowedModels = unmarshalModelEntries(allowedModelsJSON)
	return &p, nil
}
...
func scanProviders(rows *sql.Rows) ([]*model.Provider, error) {
	...
	for rows.Next() {
		...
		var latencyMs sql.NullInt64
		var checkedAt sql.NullTime
		if err := rows.Scan(&p.ID, &p.Name, &typ, &p.Config, &allowedModelsJSON, &p.CreatedBy, &p.CreatedAt,
...
func marshalModelEntries(entries []model.ModelEntry) (string, error) {
	if len(entries) == 0 {
		return "[]", nil
	}
...
func unmarshalModelEntries(raw string) []model.ModelEntry {
	if raw == "" || raw == "[]" {
		return nil
	}
