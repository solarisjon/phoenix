...
func (r *AgentRepo) List(ctx context.Context) ([]*model.Agent, error) {
	rows, err := r.db.QueryContext(ctx, `
		SELECT id, name, persona, instructions, guardrails, behaviour,
...
func (r *AgentRepo) Create(ctx context.Context, a *model.Agent) error {
	...
	_, err := r.db.ExecContext(ctx, `
		INSERT INTO agents
		  (id, name, persona, instructions, guardrails, behaviour, provider_id, model_override, can_spawn_agents, can_hire_agents, heartbeat_interval, max_concurrent, created_by, status)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		a.ID, a.Name, a.Persona, a.Instructions, a.Guardrails, a.Behaviour,
...
func (r *AgentRepo) Update(ctx context.Context, a *model.Agent) error {
	...
	_, err := r.db.ExecContext(ctx, `
		UPDATE agents SET
		  name = ?, persona = ?, instructions = ?, guardrails = ?, behaviour = ?,
		  provider_id = ?, model_override = ?, can_spawn_agents = ?, can_hire_agents = ?, heartbeat_interval = ?, max_concurrent = ?, status = ?
		WHERE id = ?`,
		a.Name, a.Persona, a.Instructions, a.Guardrails, a.Behaviour,
...
func scanAgent(row *sql.Row) (*model.Agent, error) {
	...
	err := row.Scan(&a.ID, &a.Name, &a.Persona, &a.Instructions, &a.Guardrails, &a.Behaviour,
...
func scanAgents(rows *sql.Rows) ([]*model.Agent, error) {
	...
	for rows.Next() {
		...
		var hb sql.NullInt64
		var canSpawn, canHire int
		if err := rows.Scan(&a.ID, &a.Name, &a.Persona, &a.Instructions, &a.Guardrails, &a.Behaviour,
