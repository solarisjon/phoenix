...
**Local-models Phase 2 (done 2026-08-17, #107):** prompt budgeting.
- `ModelEntry` gains `context_window`, `max_output_tokens`, `prompt_profile` (""=auto|standard|compact), `reasoning` (JSON-additive). `agent.ResolveModelProfile` (profile.go): entry → `Capable` probe → defaults; default output reserve 4096 capped at ¼ of small windows; auto profile = compact when window ≤ 16k or tier `fast`. `ModelProfile.PromptBudget()` = window − reserve − 256 − 5 %.
- `internal/agent/prompt_sections.go`: `PromptAssembly` records each injector's contribution as a `PromptSection` via `Apply()` (delta recording — Render() is byte-identical to the sequential injectors, guarded by `prompt_golden_test.go` + `testdata/prompts/*.golden`; regenerate deliberately with `-update`). `Fit(budget, count)` drops/shrinks by priority: obsidian(5) → spawn/hire(4, unless task mentions delegation) → memories(3, 50 %/25 %) → skills(2, full→outline→brief) → follow-up(1, drop oldest→truncate). Mandatory (behaviour, guardrails, task, protocol, react, orchestrator, skill mode) never trimmed; if they alone don't fit → `ErrPromptTooLarge` and the task fails with a clear message.
- `assembleSystemPrompt` is now `assembleSystemSections(…, PromptOptions{Profile})`; compact variants of memo/monitor/artifacts/spawn/hire/react live in prompt.go (`memoSection`, `artifactsSection`, `spawnSection`, `hireSection`, `InjectReactLoopInstructionsOpts`). Compact artifacts section only when the task mentions durable output.
- `buildTaskRequestMeta` returns `PromptMeta{Profile, PromptTokens, Budget, Trims}`; runner stores `tasks.prompt_tokens/prompt_trims/repair_attempts` (migration 056), emits WS `task.prompt_trimmed`, sets `req.MaxOutputTokens` to the reserve. Forced chain summarisation when the verbatim chain > 40 % of budget.
...
**Local-models Phase 4 (done 2026-08-17, #108):** structured output + repair.
- `TaskRequest.ResponseSchema` honoured by every LLM adapter: llamacpp (`response_format json_schema`, grammar-enforced), llm OpenAI flavour (`response_format json_schema`, `strict:false` to avoid OpenAI 400s on strict-mode rules), ollama (`format: <schema>`); Anthropic flavour ignores it.
- `internal/agent/schemas.go`: `PlanSchema`, `HealthSchema`, `AgentGenSchema`, `SuggestionsSchema` (object root `{"suggestions":[…]}` — prompt changed accordingly; parser accepts bare arrays too), `VaultPickSchema(names)`, `TextFieldSchema`. Set on: orchestrator request (`InjectOrchestratorInstructions`), `generateAgent`, `suggestProjectNextAction`, Obsidian vault pick.
- `internal/agent/repair.go`: `RepairStructured(ctx, prov, prevOutput, parseErr, schema, what)` — exactly one re-ask on the SAME provider. Used by: orchestrator plan (`repairPlan`, increments `tasks.repair_attempts`, sets `last_error` on double failure), `generateAgent`, suggestions, health classifier.
...
- **Assist endpoints:** never select a provider inline — call `s.assistProvider(ctx, req.ProviderID)`. If the endpoint parses JSON, set `ResponseSchema` (schemas.go), extract with `agent.ExtractJSONObject`, and allow ONE `agent.RepairStructured` pass.
- **Health signal:** `deriveHealthSignal` is marker-only; anything else goes through `Runner.classifyHealth`. Do not reintroduce keyword scanning.
